...
`execute_method` blocks direct CRUD and generic write helpers such as `web_save`, `name_create`, `copy`, `load`, and translation updates; those mutations use the validated write path. Common business side-effect method names such as `message_post`, `action_*`, `button_*`, `*_send*`, `*_post*`, and `*_validate*` are blocked unless the deployment explicitly allowlists exact methods with `ODOO_MCP_ALLOWED_SIDE_EFFECT_METHODS=model.method`. Per-user enterprise deployments can instead enable `ODOO_MCP_NATIVE_ACL_PARITY=1`: the mode only activates with strict per-user authentication and delegates positive authorization to native Odoo ACLs, record rules, and business-method validation. A small negative mutation policy can remain in `ODOO_MCP_DENIED_METHOD_PREFIXES`; it is evaluated before parity, exact allowlists, broad mode, every approved-write stage, and chatter execution. Built-in control-plane denies are not configurable. This negative mutation policy does not replace Odoo's read ACLs: read tools still execute under the current user's native credential and visibility. The older broad escape hatch, `ODOO_MCP_ALLOW_UNKNOWN_METHODS=1`, remains available and is reported separately by `health_check`.

Every mutation execution logs the tool, acting login, model, method, and parity posture without logging the API key. Caller context flags that suppress Odoo tracking are removed before execution.
